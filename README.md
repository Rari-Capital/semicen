...
- A FundDelegator contract interacts directly with a `FundManager` and `FundController` contract to calculate the amount of rebalancer fees earned and to perform  "steps" (an array of struct instances that correspond to function calls on a `FundController` contract) that are sent from the Semicen contract.

**The reason for splitting these contracts in two is for upgradability.** If a pool's liquidity is migrated to a new FundController to add functions for a new strategy, a new FundDelegator contract that adds support for that function can be created and linked to the same Semicen contract. 

This two contract solution decouples the reward and access control functionality of the Semicen contract from the logic of executing rebalance steps, which is ideal for Rari's pool system which may have constant strategy additions and updates. It is also more gas efficient as we do not have to redeploy a full new Semicen contract for each strategy update only another tiny FundDelegator, and when a FundDelegator contract is destroyed, we receive a gas refund!
